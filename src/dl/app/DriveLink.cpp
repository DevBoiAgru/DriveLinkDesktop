...
DriveLinkApp::DriveLinkApp(ResourceManager& resources)
    ...
    // Minimise button
    // Minimise DOES NOT work on linux becuase Wayland doesnt like it when apps can minimize themselves.
    // "Security"
#ifdef _WIN32
    m_uiComponents.push_back(
        std::make_unique<dl::ui::Button>(
...
void DriveLinkApp::update() {
    ...
    m_steeringSprite->setRotation(sf::radians(-inputState.getSteering() * std::numbers::pi_v<float>));
